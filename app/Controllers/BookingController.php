...
class BookingController extends BaseController
{
    ...
    public function pay(int|string $id)
    {
        ...
        $params = [
            ...
            // PERBAIKAN: Mengunci callback redirect mutlak dari backend
            'callbacks' => [
    ...
    {
        // Pastikan menangkap string query dengan bersih
        $orderId = (string) $this->request->getGet('order_id');
        $statusCode = (string) $this->request->getGet('status_code');
        ...
        if (!empty($orderId)) {
            // Gunakan pencarian trim untuk menghindari spasi tak terlihat dari URL
            $payment = $paymentModel->where('midtrans_order_id', trim($orderId))->first();

            if ($payment) {
                // Evaluasi status code Midtrans
                if ($statusCode == '200') {
                    $statusView = 'settlement';
                    $statusDb = 'paid';
                ...
                // Gunakan pencarian manual array jika update() builder mengalami delay cache
                if ($payment['status'] === 'pending') {

                    // Memicu update langsung menggunakan primary key ID tabel payments
        ...
        return view('pelanggan/bookings/finish', [
            'order_id' => $orderId ?: '-',
            'status' => $statusView
        ]);
    }
    ...
    public function approve(int|string $id)
    {
        ...
        if ($booking) {
            // Benteng Pertahanan: Jika sudah confirmed, langsung redirect balik dengan aman
            if ($booking['status'] === 'confirmed') {
                return redirect()->back()->with('success', 'Booking sudah dikonfirmasi sebelumnya.');
            }

            // 2. Eksekusi update hanya jika statusnya belum confirmed
